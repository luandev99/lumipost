...
export function IdentityCombobox({ label, options, value, onChange, placeholder = 'Pesquisar opções...', description }: BaseProps & { value: string; onChange: (value: string) => void }) {
  const [open, setOpen] = useState(false)
  const [query, setQuery] = useState('')
  const rootRef = useRef<HTMLDivElement>(null)
  const inputRef = useRef<HTMLInputElement>(null)
  ...
  return <div ref={rootRef} className="relative">
    ...
    <div className={`field flex min-h-12 items-center gap-2 !py-2 ${open ? '!border-app-primary ring-2 ring-violet-500/10' : ''}`} onClick={() => { setOpen(true); inputRef.current?.focus() }}>
      ...
      <input ref={inputRef} id={`${listId}-input`} role="combobox" aria-expanded={open} aria-controls={listId} aria-autocomplete="list" className="min-w-0 flex-1 border-0 bg-transparent py-1 text-sm outline-none placeholder:text-app-muted" placeholder={placeholder} value={open ? query : value} onFocus={() => { setQuery(''); setOpen(true) }} onChange={(event) => { setQuery(event.target.value); setOpen(true) }} onKeyDown={(event) => { if (event.key === 'Escape') setOpen(false) }} />
